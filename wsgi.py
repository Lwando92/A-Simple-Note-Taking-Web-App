...
application.secret_key = str(random.randint(1, 20))

@application.route('/')
def home_page():
    '''
        application for hompage
    '''
...
def profile():
    '''
        application for user profile can only be accessed only after successful login
    '''
            ...
            for note in notes:
                ...
                if tags_list:
                    for tag in tags_list:
                        ...
                        if temp is not None:
                            temp_list.applicationend(temp[0])
                tags.applicationend(', '.join(temp_list))
...
def login():
    '''
        application for creating Login page
    '''
    form = LoginForm()
...
def signup():
    '''
        application for registering new user
    '''
    form = SignUpForm()
    if form.validate_on_submit():
        username = request.form['username']
...
def logout():
    '''
        application for logging out user
    '''
...
@application.route("/notes/add/", methods=['GET', 'POST'])
@login_required
def add_note():
    '''
        application for adding note
    '''
    form = AddNoteForm()
...
@application.route("/notes/<id>/")
@login_required
def view_note(id):
    '''
        application for viewing a specific note
    '''
    notes = functions.get_data_using_id(id)
    return render_template('view_note.html', notes=notes, username=session['username'])


@application.route("/notes/edit/<note_id>/", methods=['GET', 'POST'])
@login_required
def edit_note(note_id):
    '''
        application for editing a particular note
    '''
    form = AddNoteForm()
    form.tags.choices = functions.get_all_tags(session['id'])
...
@application.route("/notes/delete/<id>/", methods=['GET', 'POST'])
@login_required
def delete_note(id):
    '''
        application for viewing a specific note
...
@application.route("/tags/add/", methods=['GET', 'POST'])
@login_required
def add_tag():
    '''
...
def view_tag():
    '''
        application for viewing all available tags
    '''
    tags = functions.get_all_tags(session['id'])
    return render_template('edit_tag.html', tags=tags, username=session['username'])


@application.route("/tags/view/<tag_id>")
@login_required
def view_notes_using_tag(tag_id):
    '''
        application for viewing all available notes tagged under specific tag
    '''
...
@application.route("/tags/delete/<tag_id>/")
@login_required
def delete_tag(tag_id):
    '''
        application for deleting a specific tag
    '''
...
@application.route("/profile/settings/")
@login_required
def profile_settings():
    '''
        application for getting profile settings for a user
    '''
...
@application.route("/profile/settings/change_email/", methods=['GET', 'POST'])
@login_required
def change_email():
    '''
...
@application.route("/profile/settings/change_password/", methods=['GET', 'POST'])
@login_required
def change_password():
    '''
...
def background_process():
    '''
        application for handling AJAX request for searching notes
    '''
    try:
